...
impl SlashCommand for BanCommand {
    ...
    fn build(&self) -> Command {
        ...
        .option(UserBuilder::new("user", "The user to ban").required(true))
        .option(StringBuilder::new("reason", "Reason for the ban"))
        .option(IntegerBuilder::new(
            "days",
            "Delete messages from last N days (0-7)",
        ))
        .build()
    }
    ...
    async fn execute(&self, ctx: &SlashCommandContext) -> BotResult<()> {
        ...
        for opt in &ctx.data.options {
            ...
            if opt.name == "days" {
                if let twilight_model::application::interaction::application_command::CommandOptionValue::Integer(i) = opt.value { days = Some(i); }
            }
        }
        ...
        let confirmation_text =
            Self::build_confirmation_text(target.get(), reason.as_deref(), days);
        let container = Self::build_container(&confirmation_text);

        let confirm_id = format!("mod_confirm:ban:{}:{}", guild_id.get(), target.get());
